define([
        ...
        (function(){
            ...
            $plus.click(function () {
                ...
                var chenks = $(this).closest('.order_lists ').find('.son_check');
                 cyCheck(chenks,allarr,dts);
                 cyAlls(inventory,mz_nid,$count)
            });
            ...
            function cyAlls(inventory,mz_nid,count){
                $.ajax({
                    url:editacr_url,
                    type:'post',
                    data:{
            ...
            function cyCheck(chenks,allarr,dts){
    ...
    exports.manageAddress = function(){
        (function(){
            ...
            function cyVal(_brnTHis){
                var user_name =  _brnTHis.closest('.receiver_details_head').find('.user_name').text();
                var phones =  _brnTHis.closest('.receiver_details_head').find('.user_phone').text();
                var user_pr =  _brnTHis.closest('.receiver_details_head').find('.user_pr').text();
                var user_pr_val =  _brnTHis.closest('.receiver_details_head').find('.user_pr').attr('data-id');
                var user_city =  _brnTHis.closest('.receiver_details_head').find('.user_city').text();
                var user_city_val =  _brnTHis.closest('.receiver_details_head').find('.user_city').attr('data-id');
                var user_qu =  _brnTHis.closest('.receiver_details_head').find('.user_qu').text();
                var user_qu_val =  _brnTHis.closest('.receiver_details_head').find('.user_qu').attr('data-id');
                var user_jt =  _brnTHis.closest('.receiver_details_head').find('.user_jt').text();
                            ...
                            success:function(data){
                                if(data){
                                    ...
                                    if($('.receiver_details').length==1){
                                        ...
                                        var lengths =  $('.receiver_details_head').length;
                                        $('.has_receive').text('已保存收货地址（共'+lengths+'条）最多保存10条');
                                    }
            ...
            $('.receiver_details').on('mouseenter',function(){
                $(this).find('.default_addr').show();
            })
            $('.receiver_details').on('mouseleave',function(){
                $(this).find('.default_addr').hide();
            })
            //设为默认
            ...
            function clear(){
                $('#m_username').val('');
                $('#m_phone').val('');
                var city_txt = $('#city').find('option:selected').text();
                var city_val = $('#city').find('option:selected').val();
                var xian_txt =  $('#xian').find('option:selected').text();
                var xian_val =  $('#xian').find('option:selected').val();
                
                $('#city').find('option:first-child').val(city_val);
                $('#city').find('option:first-child').text(city_txt);
                $('#xian').find('option:first-child').val(xian_val);
                $('#xian').find('option:first-child').text(xian_txt);

                $('#city').find('option:selected').text('请选择');
                $('#city').find('option:selected').val('请选择');
                $('#xian').find('option:selected').text('请选择');
                $('#xian').find('option:selected').val('请选择');
    ...
    exports.appplyRefund = function(){
        (function(){
            ...
            $('.moer_tui').on('keyup',function(){
                ...
                // var money = $(".price").html()*$(".goods_count").html();
                var money = nall;
                if(val>money){
